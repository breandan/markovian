...
\begin{itemize}
\item Probabilistic inference requires a lot of messy pen-and-paper derivation
...
\item Is it possible to use logical inference to automate probabilistic inference?
\end{itemize}
...
There are various methods for combining probability distributions. For independent RVs, we might use the joint distribution or the convolution of the distributions:
...
For dependent RVs, we need to specify the dependence relation. In general, to combine two arbitrary RVs related by some dyadic function $f$, we need Fubini-Tonelli:

\vspace{\baselineskip}

% https://terrytao.wordpress.com/2015/10/12/275a-notes-2-product-measures-and-independence/
\begin{prooftree}
\AxiomC{$x \sim P(X)$, $y \sim P(Y)$}
\AxiomC{$f: X \times Y \rightarrow Z$}
\AxiomC{$z = f(x, y)$}
\TrinaryInfC{$P(Z = z) = \int_{X \times Y} f(x, y)d(x \times y)$}
\RightLabel{FuTon}
...
\AxiomC{$\Gamma \vdash P(X, Y): X\times Y\rightarrow\mathbb{R}^+$}
\RightLabel{Margin}
\UnaryInfC{$\Gamma \vdash P(X) \propto \int_{Y} P(X \mid Y = y)dy$}
\end{prooftree}
