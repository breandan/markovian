...
\begin{itemize}
    ...
    \item [(5)] $P(X + Y) = P(X) + P(Y), \forall X \cap Y = \varnothing$.
\end{itemize}
...
So far, we have defined the denotational semantics of probabilities. But what about the operational semantics? If we have two distributions, how do we combine them to obtain numerical values? To implement a probabilistic combinator, we need the concept of \textit{convolution}, the sum of a product at each point:

\begin{prooftree}
    \AxiomC{$\Gamma \vdash g: S^d \rightarrow \mathbb{R}, h: S^d \rightarrow \mathbb R$}
    \RightLabel{DiscConv}
    \UnaryInfC{$(g * h)(\mathbf x) = \int_{\mathbb R^d} f(\mathbf y)g(\mathbf{x} - \mathbf{y})d\mathbf{y}$}
    \DisplayProof

    \AxiomC{$\Gamma \vdash g: S^d \rightarrow \mathbb{Z}, h: S^d \rightarrow \mathbb Z$}
    \RightLabel{ContConv}
    \UnaryInfC{$(g * h)(\mathbf x) = \sum_{\mathbb R^d} f(\mathbf y)g(\mathbf{x} - \mathbf{y})d\mathbf{y}$}
\end{prooftree}

For two independent random variables $X$ and $Y$, their sum can be defined as convolution:

\begin{prooftree}
    \AxiomC{$\Gamma \vdash P(X), P(Y)$}
    \AxiomC{$Z = X + Y$}
    \RightLabel{Conv}
    \BinaryInfC{$P(Z) = P(X) * P(Y)$}
\end{prooftree}

Notably, the sum of the distributions is \textit{not} equivalent to the sum of their random variables.
